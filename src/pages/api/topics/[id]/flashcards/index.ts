...
import type { FlashcardResponseDto, FlashcardsResponseDto } from '@/types';

// Simple in-memory cache for flashcard data to reduce database queries
const flashcardCache: Record<string, { data: any[], timestamp: number }> = {};
const CACHE_TTL = 60000; // 1 minute cache TTL

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<FlashcardsResponseDto | any>
) {
  ...
  switch (req.method) {
    case 'GET':
      try {
        ...
        if (flashcardCache[topicId] && 
            (Date.now() - flashcardCache[topicId].timestamp) < CACHE_TTL) {
