...
export default function ManualFlashcardCreationView({ topicId }: ManualFlashcardCreationViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await processFlashcardAccept(topicId, question, answer, "manual", supabaseClient);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Create flashcards by filling out the form below.</p>
      </div>
      ...
      <Card className="w-full">
        <form onSubmit={handleSubmit} data-test-id="add-flashcard-form">
          ...
          <CardFooter className="flex justify-between">
            <Button variant="secondary" type="button" onClick={() => (window.location.href = `/topics/${topicId}`)}>
              Cancel
            </Button>
            <CreateFlashcardButton type="submit" disabled={isSubmitting}>
