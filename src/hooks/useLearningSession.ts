...
export function useLearningSession(topics: TopicSummaryDto[] = []) {
    ...
    try {
      ...
      try {
        ...
        if (!sessionResponse.ok) {
         ...
         setError(sessionErr instanceof Error ? `Session creation failed: ${sessionErr.message}` : 'Session creation failed');
  ...
  const ratingMap: { [key in 'bad' | 'medium' | 'good']: 'Again' | 'Hard' | 'Easy' } = {
    bad: 'Again',
    medium: 'Hard',
    good: 'Easy',
  };

  const rateCard = useCallback(async (rating: 'bad' | 'medium' | 'good') => {
    const cardToRate = flashcards[currentCardIndex];
    const userResponse = ratingMap[rating]; // Map to backend value ('Again', 'Hard', 'Easy')

    console.log(`Card rated: ${rating} (mapped to: ${userResponse}), Session ID: ${sessionId}, Card ID: ${cardToRate?.id}`);
  ...
  }, [flashcards, currentCardIndex, sessionId]); // Dodano brakującą tablicę zależności i nawias zamykający dla useCallback
