...
const handleApiError = async (response: Response, defaultMessage: string): Promise<string> => {
  ...
  try {
    ...
    const errorData: ErrorResponse | { error: string; details?: unknown } | { message: string } = await response.json();
    ...
    if (typeof errorData === "object" && errorData !== null) {
      ...
      if (
        response.status === 400 &&
        "error" in errorData &&
        errorData.error === "Invalid request body" &&
...
export const useAIGeneration = (topicId: string) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const generatedResponse: { flashcards: { front: string; back: string }[] } = await response.json();
  ...
  const handleAccept = useCallback(
    async (suggestionId: string) => {
      ...
      try {
        ...
        const result: FlashcardAcceptResponseDto = await processFlashcardAccept(
          topicId,
          suggestionToAccept.front,
          suggestionToAccept.back,
          "ai-generated",
          supabaseClient
        );
        if (!result) {
  ...
  const handleRegenerate = useCallback(
            ...
            (s) =>
              s.id === suggestionId
                ? {
                    ...
                    back: regeneratedData.back,
                    exceeds_limit: regeneratedData.exceeds_limit,
                    originalFront: regeneratedData.front,
                    originalBack: regeneratedData.back,
                    isEditing: false,
                  }
    ...
    [suggestions, topicId, sourceText]
  ); // Include sourceText if needed by API

  const handleEditToggle = useCallback((suggestionId: string) => {
    setSuggestions((prev) => prev.map((s) => (s.id === suggestionId ? { ...s, isEditing: !s.isEditing } : s)));
  }, []);

  const handleSaveEdit = useCallback(
    async (suggestionId: string, editedFront: string, editedBack: string) => {
      setIsLoading(true); // Consider more granular loading
      setError(null);
